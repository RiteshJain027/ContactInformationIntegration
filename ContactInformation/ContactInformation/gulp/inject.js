...
gulp.task('inject', ['scripts', 'styles', 'injectAuth', 'inject404', 'injectUnauth', 'copyVendorImages'], function () {
    var injectStyles = gulp.src([
        path.join(conf.paths.tmp, '/serve/app/main.css'),
        path.join('!' + conf.paths.tmp, '/serve/app/vendor.css'),
        path.join(conf.paths.src, '/node_modules/bundles/ng-table.css')
    ], { read: false });
    ...
    var injectScripts = gulp.src([
        ...
        path.join('!' + conf.paths.src, '/app/ng-table/bundles/.mock.js'),
        path.join(conf.paths.src, '/node_modules/bundles/ng-table.js')
    ])
...
gulp.task('injectUnauth', ['styles404'], function () {
    return injectAlone({
        css: [path.join('!' + conf.paths.tmp, '/serve/app/vendor.css'), path.join(conf.paths.tmp, '/serve/app/404.css')],
        paths: [path.join(conf.paths.src, '/unauth.html')]
    })
});
